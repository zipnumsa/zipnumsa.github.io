...
// ### 따옴표 문자에 대한 처리 옵션 및 임시 보관용 전역 변수 선언 ####################################################
...
// ### 제목으로 그 제목을 가진 데이터 파일명을 찾아 돌려주는 함수 ####################################################
function wkfunc_file_find_from_title($title_parameter)
{
	...
	return $file_name_return.$file_number_counter; // 그런 제목을 갖는 페이지가 없을 경우, "__NONEXISTANCE__전체검색한파일갯수" 를 리턴한다.
}

// ### 주어진 시간을 가지고, 그 시간 이후로 최신 업데이트 파일의 파일명을 찾아 돌려주는 함수 #########################
...
// ### 주어진 문자열을 가지고, 그 문자열보다 사전순으로 바로 뒤쪽에 있는 제목을 가진 파일명을 찾아 돌려주는 함수 #####
...
function wkfunc_make_title_sorted_list(){
	...
	&&	strpos($file,".xml")<1 
		...
		$temporal_title    = str_replace( "\n","",$temporal_title);
		$temporal_title    = str_replace( "\r","",$temporal_title);
		if(strcmp($temporal_title,"삭제")==0) continue;
...
// ### stack 문자열에서 needle 문자열이 사전순으로 배열하면 어디에 끼여야 하는지 문자열 인덱스를 찾아주는 함수 #######
...
// ### 아무 데이터 파일이 없으면 디자인 파일부터 자동생성 한다. ######################################################
...
if(strpos($HTTP_GET_VARS["option"],"tatic")==1){ // static 옵션이 있을 경우, 다른 디자인 파일을 읽는다.
...
if( strlen($page_to_read) < 1) {
	...
	// 인터위키 페이지를 요구하며 페이지를 요청할 경우 자동으로 리다이렉션하며 종료
	for($ii=0;$ii<4;$ii++){
...
if( strlen($page_to_read) < 1 || strcmp($page_to_read,"FIRSTPAGE")==0) $wk_title = $wk_first_page_title;
	...
	// 디자인 파일에 대한 단순 가공
	$wk_design = str_replace("메인페이지제목",$wk_first_page_title, $wk_design);
	$wk_design = str_replace("처음페이지제목",$wk_first_page_title, $wk_design);
	$wk_design = str_replace("위키키위제목",str_replace("_"," ",$wk_title), $wk_design);
	$wk_design = str_replace("페이지고치기링크","index.php?pagetoedit=".$wk_title."&option=oekaki",$wk_design);
	$wk_design = str_replace("역링크페이지링크","index.php?pagetoread=SRESULT&sphrase=[ ".$wk_title." ]",$wk_design);
	$wk_design = str_replace("페이지고치기","index.php?pagetoedit=".$wk_title."&option=oekaki",$wk_design);
	$wk_design = str_replace("텍스트고치기링크","index.php?pagetoedit=".$wk_title,$wk_design);
	$wk_design = str_replace("트랙백보내기링크","index.php?pagetoread=".$wk_title."&option=trackback",$wk_design);	
	$wk_design = str_replace("달라진점보기링크","index.php?pagetoread=".$wk_title."&option=diff",$wk_design);	
	$wk_design = str_replace("옛날판보기링크","index.php?pagetoread=".$wk_title."&option=history",$wk_design);	
	$wk_design = str_replace("정적HTML생성링크","index.php?pagetoread=".$wk_title."&option=static",$wk_design);	
	$wk_design = str_replace("처음페이지링크","index.php?pagetoread=FIRSTPAGE",$wk_design);
	$wk_design = str_replace("목록페이지링크","index.php?pagetoread=TITLELIST",$wk_design);
	$wk_design = str_replace("업데이트페이지링크","index.php?pagetoread=UPDATELIST",$wk_design);
	$wk_design = str_replace("파일업로드링크","index.php?pagetoread=UPLOAD",$wk_design);
	$wk_design = str_replace("검색페이지링크","index.php?pagetoread=SEARCH",$wk_design);
	$wk_design = str_replace("도움말페이지링크","index.php?pagetoread=위키키위%20도움말",$wk_design);
	$wk_design = str_replace("새로운페이지링크","index.php?pagetoedit=제목입력&option=oekaki", $wk_design);
	$wk_design = str_replace("위키키위RSS링크","index.xml",$wk_design);
...
// ### 디자인 파일에 기록된 요소의 parsing ###########################################################################
$position_of_contents = strpos($wk_design,"페이지내용");
$position_of_headline = strpos($wk_design,"헤드라인소제목");
$position_of_blit = strpos($wk_design,"나열블릿");
$headline_length = strlen("헤드라인소제목");
...
if( strcmp($page_to_edit,"FIRSTPAGE") == 0 ) { // 첫 페이지일 경우 볼 것도 없이 wkct0.txt 파일에 대한 편집임은 확실하다.
...
if(strpos($filename_to_edit,"_NONEXI")>0) {$createnew=1; $filename_to_edit="wkct".substr($filename_to_edit,16).".txt";} // 고치고자하는 제목의 페이지가 없는 경우, 새로운 이름으로 파일 생성
...
$wk_contents = str_replace("\r","",$wk_contents);
	...
	// 수정 제한 암호(혹은 답글만 허용)가 걸려 있는 경우의 처리
$i = substr($wk_contents,0,5);
if((strcmp($i,"암호 ")==0 || strcmp($i,"앞글 ")==0  || strcmp($i,"답글 ")==0)  && strcmp($j,"reply") != 0)
{
	$i = strpos($wk_contents,"\n");
	$wk_password = substr($wk_contents,5,$i-5);
	if( strcmp($HTTP_GET_VARS["pwd"],$wk_password)!=0) { // 암호를 제공 받지 못했거나, 암호가 다른 경우, 암호를 물어보는 창을 띄움
	...
	echo "</script>\n";
	exit();
	} // 암호가 제공되었으며, 일치하는 경우 일반적인 수정 작업으로 이어짐.
}

echo $wk_head;
echo "<form method=post action=index.php>\n";
$redirection_home_url = $HTTP_GET_VARS["homeurl"];
if( ( strcmp($i,"앞글 ")==0  && strcmp($j,"eply") == 1) || ( strcmp($i,"답글 ")==0  && strcmp($j,"eply") == 1) || strcmp($j,"eply") == 1 )
{ // 덧글 류일 경우, 띄우는 form의 형태가 다름
...
// ### 오에카키를 위한 자바스크립트를 덧붙인다. ######################################################################
// ### 전혀 사용하지 않거나 대체할 경우 이곳을 편집. #################################################################
...
// ### 페이지 내용을 실제로 파일에 쓰는 부분 #########################################################################
// ### 페이지 편집을 한 후 submit 결과로 요청하게 되는 페이지는 바로 이 부분이 된다. #################################
// ### 트랙백도 이곳에서 처리함. #####################################################################################
$page_to_read = "";
$page_to_edit = "";
...
// ### 트랙백처리 ###################################################################################################
if(strlen($page_to_edit)<1) { // blog_name이 설정된, POST가 아닌 get방식으로만 파일이름을 정하는 요청은 트랙백처리임
...
$HTTP_POST_VARS["exceprt"]=str_replace("]","",$HTTP_POST_VARS["excerpt"]);
	...
	} // 트랙백은 일종의 답글로 처리된다. 즉, 트랙백을 보내기 위해 날아온 내용들은 답글을 위해 전달된 정보인 것 처럼 변환되고, 일반 답글처리로 이어지게 된다.
}
...
if(strcmp($reply_edit_type,"reply")==0) // 만약 답글이라면,
{
	...
	if( strcmp($i,"암호 ")==0 || strcmp($i,"앞글 ")==0  || strcmp($i,"답글 ")==0 ) {
            ...
            for($l=0;$l<300;$l++){
            	$main_temp = wkfunc_first_title_file_find($main_temp2);
             	if(strcmp($main_temp,$main_temp2)==0 || strlen($main_temp)<2 ) break;
              	if(strcmp($main_temp,"삭제")!=0 && strcmp($main_temp,$wk_title)!=0) {
...
if(strlen($wk_contents)>2 && strlen($wk_title)<40 ) { // 해킹, 스팸 및 치명적 에러 방지를 위해
	//if(strlen($HTTP_POST_VARS["name"])>1 && strpos($HTTP_POST_VARS["nospam"], "ehehe")!=1) exit(1); // spam 방지
	...
	//	exit(1); // 스팸 방지
	// }
	if(strlen($HTTP_POST_VARS["name"])<2 && strcmp($reply_edit_type,"reply")==0 ) exit(1); // spam 방지
...
fclose($fp);
	...
	// XML list setup
	//
// ### Ajax static backup 페이지를 위한 XML 파일리스트의 작성 ########################################################
	$xmllist = "";
	if(file_exists("wkctlist.xml")!=false){
	$fp = fopen( "wkctlist.xml", "rb" );
        $xmllist = fread( $fp, filesize( "wkctlist.xml" ) );
...
for($l=0;$l<7;$l++){
	...
	$fixedtitle = "";
	if(strcmp($main_temp,"삭제")!=0) $fixedtitle =  substr($main_temp,$j+1,$k-$j-1);
        if(strcmp($main_temp,"삭제")!=0 && strcmp($fixedtitle,$wk_first_page_title_noret)!=0)
        {
...
// ### 작업 완료후 클라이언트에 표시할 메세지들 ######################################################################
	$tb_url = urlencode( $tb_url );
	$page_to_read = $wk_title;
	if(strpos($HTTP_POST_VARS["url"],"://")>0 || strcmp($HTTP_GET_VARS["xml"],"yes")==0) { // track back을 보내거나 xml로 답해 오라는 요청이 있었다면, 그대로.
	...
	if(strlen($tb_url)>3) 
		...
		if(strlen($HTTP_POST_VARS["homeurl"])<2) // 이도 저도 아닌 경우 homeurl이라는 변수로 넘어온 주소 혹은, 쓰기 완료한 문제의 페이지로 리다이렉션
...
echo "<BR><input type=checkbox name=autolinkgen value=false>자동링크생성\n";
echo "</form>";
echo $wk_tail;
...
// Titlelist
if(strcmp($wk_title,"제목 순서 목록")==0 || strcmp($wk_title,"Title List")==0) {
$wk_date ="";
$wk_contents = "\n";
$main_temp2 = "";
$i = 0;
...
else if(strcmp($wk_title,"검색 결과")==0 || strcmp($wk_title,"Search Result")==0) {
$wk_date ="";
$wk_contents = "\n";
$main_temp2 = "";
$main_title_temp = "";
...
else if(strcmp($wk_title,"검색")==0  || strcmp($wk_title,"Search")==0) {
...
$wk_contents = $wk_contents.'<input type="submit" value="검색">';
$wk_contents = $wk_contents.'</form>';
}


else if(strcmp($wk_title,"파일 업로드")==0  || strcmp($wk_title,"File Upload")==0) {
...
$wk_contents = $wk_contents.'<input type="submit" value="업로드">';
$wk_contents = $wk_contents.'</form>';
}

else if(strcmp($wk_title,"파일 업로드 완료")==0  || strcmp($wk_title,"Upload Completed")==0) {
	$uploaddir = './';
	$uploadfile = $uploaddir. $_FILES['userfile']['name'];

	if (move_uploaded_file($_FILES['userfile']['tmp_name'], $uploadfile)) {
		$wk_contents = $wk_contents."파일이 존재하고, 성공적으로 업로드 되었습니다.";
		$wk_contents = $wk_contents."추가 디버깅 정보입니다:\n";
                echo("<!--");print_r($_FILES);echo("-->");
	} else {
		$wk_contents = $wk_contents."파일 업로드 공격의 가능성이 있습니다! 디버깅 정보입니다:\n";
                echo("<!--");print_r($_FILES);echo("-->");
	}
...
//  Update List
else if(strcmp($wk_title,"최근 업데이트 페이지")==0  || strcmp($wk_title,"Updated Pages")==0) {
$wk_date ="";
$wk_contents = "\n";
$main_temp2 = "";
$i = 0;
...
// If it's not
	...
	$lines = explode("\n",$wk_contents1);
	$nondeco_wk2 = "<h2>수정 및 추가된 부분 표시</h2><BR>";
	...
	$nondeco_wk2 = $nondeco_wk2."<BR><h2>수정 이전 버전의 변경 부분</h2><BR>";
...
if( strpos($wk_contents,"[IncludeUpdate") != false )
{
	...
	if(strpos($include_title,"ikiSandBox")==1 || strpos($include,"기메모와 테스트")>0) {
...
if(strpos($inc_wk_firstline,"앞글 ")==1 || strpos($inc_wk_firstline,"답글 ")==1)
...
fclose( $fp );
	...
	if($k != false)
	{
		...
		// Processing contents
if(
strcmp($wk_title,"파일 업로드")!=0 && strcmp($wk_title,"파일 업로드 완료")!=0 && strcmp($wk_title,"검색")!=0 && 
...
echo '트랙백 보낼 주소 <input name=actiontarget type=text size=60 value="'.$HTTP_GET_VARS["target"].'"><BR>';
echo '제목 <input name=title type=text value="'.$wk_title.'">';
echo '<input name=blog_name type=hidden value="'.$wk_first_page_title.'"><br>';
echo '내용요약 <textarea name=excerpt>more...</textarea><BR>';
echo '<input name=url type=hidden value="http://'.$css_URL_title."index.php?pagetoread=".urlencode($page_to_read).'">';
echo '<input name=go value="보내기" type=button onclick="gotrackback()">';
echo '</form>';
echo '<script language="javascript">';
...
$tmp_title = str_replace("\r","",$wk_title);
$tmp_title = str_replace("\n","",$tmp_title);
if(file_exists("wkctlinks.xml")!=false && strpos($wk_design,"역링크들표시")>0){
	$fp = fopen( "wkctlinks.xml", "r" );
	fgets($fp); 
	$tmp_links = fgets($fp); 
	fclose($fp);
...
$wk_design = str_replace("마지막변경일시",$wk_date, $wk_design);
$wk_design = str_replace("LastEditedTime",$wk_date, $wk_design);
			// Backlink processing
$wk_design = str_replace("역링크들표시",$converting_back_links,$wk_design);
...
if( (strcmp($filename_to_read,"wkct0.txt")==0 || strcmp($HTTP_GET_VARS["pagetoread"],"FIRSTPAGE")==0) && strcmp($include_title,"nothing")!=0)
	{
	$wk_design = str_replace("덧글달기링크","index.php?pagetoedit=".$include_title."&edittype=reply",$wk_design);
	...
	{
	$wk_design = str_replace("덧글달기링크","index.php?pagetoedit=".$wk_title."&edittype=reply",$wk_design);
...
$css_URL_title_hostname = $GLOBALS['HTTP_HOST'];
$css_URL_title = $css_URL_title_hostname . substr($css_URL_title_URI, 0, strpos($css_URL_title_URI,"index.php"));
$wk_design = str_replace("트랙백주소표시", "http://".$css_URL_title."index.php?pagetowrite=".$filename_to_read, $wk_design);
...
$wk_design = str_replace("==&gt; 엌정늬 &lt;==", "<center><img src='emo_ejn.jpg'></center>", $wk_design);
...
$wk_design = str_replace("{그러췌}"," <img src=\"thumbs-up.gif\"> ",$wk_design);
...
$wk_design = str_replace(" [앞글달기]"," <a href=\"index.php?pagetoedit=".$wk_title."&edittype=reply\">답글달기</a>",$wk_design);
$wk_design = str_replace(" [답글달기]"," <a href=\"index.php?pagetoedit=".$wk_title."&edittype=reply\">답글달기</a>",$wk_design);
$wk_design = str_replace("\n[앞글달기]","<a href=\"index.php?pagetoedit=".$wk_title."&edittype=reply\">답글달기</a>",$wk_design);
$wk_design = str_replace("\n[답글달기]","<a href=\"index.php?pagetoedit=".$wk_title."&edittype=reply\">답글달기</a>",$wk_design);
